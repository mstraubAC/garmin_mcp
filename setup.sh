...
#   4. Installs pre-commit hooks (commit-stage: ruff, mypy, whitespace, YAML/JSON checks)
#   5. Installs pre-push hooks (full test suite with coverage check)
#
...
echo ""
echo "==> Installing pre-commit hooks (commit stage)..."
uv run pre-commit install --install-hooks

echo ""
echo "==> Installing pre-commit hooks (push stage: tests + coverage)..."
...
echo ""
echo "    On every commit:     ruff lint + format, mypy, whitespace, YAML/JSON checks"
echo "    On every push:       full test suite with coverage >= 78%"
